...
export const onRequestPatch: PagesFunction<Env> = async ({ request, env, params }) => {
  ...
  for (const key of ["title", "description", "due_date", "priority", "assignee", "column_id", "content_type", "platform", "card_type", "sort_order", "warning_hours", "start_date", "board_id", "color"]) {
    if (key in body) {
      fields.push(`${key} = ?`);
